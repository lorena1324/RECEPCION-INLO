...
function renderUbicacion() {
  ...
  for (let n = 1; n <= numMuelles; n++) {
    ...
    // vehículo Y de cómo viene la mercancía: la misma mula tarda
    // 3h30 arrumada y 45 minutos paletizada.
        ...
        <div class="muelle-card-body">
          ${r
            ? `<div class="muelle-card-placa">${escapar(r.placa)}</div><div>${escapar(r.conductor || "—")}</div>` +
              `<div style="margin-top:4px;"><span class="badge badge-canal">${escapar(r.canal || "—")}</span></div>` +
...
/* =========================================================
   CÓMO VIENE LA MERCANCÍA Y LA ALERTA DEL MUELLE

   J3 distingue si la mercancía viene arrumada (bulto a bulto) o
   paletizada (en estibas), porque la misma mula tarda 3h30
   arrumada y 45 minutos paletizada: medir las dos contra la misma
   meta convierte el indicador en ruido.

   Lo marca el supervisor desde aquí y no la portería, porque es
   quien ve el camión abierto — en la fila de entrada no siempre
   se sabe. Mientras nadie lo marque rige ARRUMADO, que es el caso
   más lento: equivocarse hacia ahí da una alerta tarde, y hacia
   el otro lado daría alarmas falsas en todos los vehículos que
   nadie alcanzó a marcar.
...
function openModalNovedades(id) {
  ...
  /* La ficha compartida, la misma que ven el administrador y el
     supervisor de J4. Este modal armaba a mano cinco filas —placa,
     conductor, ubicación, ingreso y programación— y por eso J3, que
     es justamente la bodega que distingue cómo viene la mercancía,
     no mostraba esa modalidad en ninguna parte: el único rastro era
     la línea del historial de aquí abajo, que se lee como una nota
     de alguien y no como un dato del vehículo.
...
/* El puesto en la fila ya no se decide con el mismo reloj para
   ...
   tipología —y en J3, contra la que corresponde a cómo viene la
   mercancía— y el que espera afuera contra el límite de patio. El
