...
const CLIENTE_POR_DEFECTO = 'Alkosto';
...
/* Los números REALES de los muelles, en orden. En J4 son el 9, el
   ...
   llaman 1, 2 y 3. Todo lo que pinte muelles recorre esta lista y
   no un contador de 1 a N. */
...
/* No todas las bodegas cobran. Donde no se cobra, la ficha del
   vehículo no lleva bloque de pago: un "Pendiente por registrar"
   permanente en una portería que nunca cobra es una alarma falsa. */
...
/* =========================================================
   ...
   Dos relojes distintos y ninguno escrito ya en el código:

     PATIO    la espera es igual para todos — esperar es esperar.
              El límite lo pone el administrador por bodega.

     MUELLE   aquí sí depende del vehículo: la meta sale de su
              tipología y de la FASE en la que va (descargue o
              cargue). Una MULA 40 arrumada no debería tardar lo
              mismo que una paletizada.

   En los dos casos: pasarse de la meta enciende el amarillo,
   doblarla enciende el rojo.
   ========================================================= */
...
/* La línea que dice cuánto lleva en muelle y contra qué meta. Se
   muestra siempre que haya meta, no solo al pasarse: el operario
   necesita ver que va en 40 de 105 minutos para saber que va bien,
   no enterarse solo cuando ya es tarde.
...
/* Configuración de la bodega (config/J4). De aquí sale la lista de
...
function renderDashboard() {
    ...
    // Banner de espera en patio. El umbral era 240 minutos escritos
    // aquí sueltos: el administrador podía mover el límite de patio
    // de la bodega y este aviso seguía saltando a las 4 horas.
    var urgentePatio = umbralesDePatio().urgente;
    var retrasados = enPatio.filter(function (r) { return minutosEnPatio(r) >= urgentePatio; });
    ...
    if (retrasados.length) {
        ...
        document.getElementById('alerta-patio-detalle').textContent =
            retrasados.length + ' vehículo(s) han superado ' + formatDuration(urgentePatio) + ' en patio: ' +
            retrasados.map(function (r) { return r.placa; }).join(', ');
    } else {
        banner.style.display = 'none';
    }

    // Banner de tiempo en muelle. Es OTRA alerta, no la misma con
    // otro número: la de arriba mide la espera en patio contra el
    // límite general de la bodega, y esta mide lo que cada vehículo
    // lleva EN MUELLE contra la meta de SU tipología. Un vehículo
...
/* =========================================================
   ALERTA DE TIEMPO EN MUELLE

   La segunda alerta del tablero, y separada de la de patio a
   ...
   su tipología para la fase en la que va.

   Solo entran los que tienen meta configurada: ver
   enMuelleFueraDeMeta() en eventos.js.
   ========================================================= */
...
function poblarSelectMuelles(selectEl, muelleActual) {
    ...
    var enMuelle = getRegistrosEnMuelle(registros);
    var ocupacion = getMuellesOcupacion(enMuelle, numerosMuelle);
    var libres = getMuellesLibres(ocupacion, numerosMuelle, muelleActual);
...
function openModalDetalle(id) {
    ...
    document.getElementById('modal-detalle-body').innerHTML =
        fichaVehiculo(rec, {
            ...
            // El monto NO: el operario ve si el pago quedó registrado
            // y nada más. Ver el encabezado de cobros.js.
...
function exportarTodos() {
    var ok = exportarExcel(registros, getStateLabel, 'inlotrans_' + OPERACION + '_' + today() + '.xlsx', 'Registros ' + OPERACION, etiquetasCampos());
...
function exportarHoy() {
    ...
    var ok = exportarExcel(deHoy, getStateLabel, 'inlotrans_' + OPERACION + '_hoy_' + hoy + '.xlsx', 'Hoy', etiquetasCampos());
