...
const MUELLES_POR_DEFECTO = 3;
const HORA_CORTE_POR_DEFECTO = 6;

let numMuelles = MUELLES_POR_DEFECTO;

/* Los números REALES de los muelles, en orden. En J4 son el 9, el
   10 y el 11: la bodega tiene tres, pero dentro de la planta no se
   llaman 1, 2 y 3. */
let numerosMuelle = numerosDeMuelle(null, MUELLES_POR_DEFECTO);
...
function pintarTituloMuelles() {
  ...
  if (!el) return;
  if (!numerosMuelle.length) { el.textContent = "Muelles"; return; }
  el.textContent = `Muelles (${numerosMuelle[0]} a ${numerosMuelle[numerosMuelle.length - 1]})`;
}
...
/* =========================================================
   ALERTAS: DE DÓNDE SALEN LOS UMBRALES

   Dos relojes distintos, cada uno con su meta, y ninguna de las
   dos vive ya escrita en el código:

     PATIO    la espera es igual para todos, sin distinción de
              tipología — esperar es esperar. El límite lo pone
              el administrador por bodega (`limitePatio`).

     MUELLE   aquí sí depende del vehículo: cada tipología tiene
              su meta, y una MULA 40 arrumada no debería tardar
              lo mismo que una MULA 20 paletizada. La meta sale
              de la tipología del vehículo y de la FASE en la que
              va (descargue o cargue), que es como se configuró.

   En los dos casos la regla es la misma: pasarse de la meta
   enciende el amarillo, doblarla enciende el rojo.
   ========================================================= */

function umbralesDePatio() {
  return configBodega ? umbralesPatio(configBodega) : null;
}

/* El nivel de alerta de un vehículo que está en muelle, contra la
   meta de SU tipología. Devuelve 'normal' si no tiene tipología o
   si esa tipología no tiene meta para su fase: no se puede
   incumplir una meta que nadie configuró. */
/* =========================================================
...
function bloqueClasificacion(r) {
  ...
  const falta = !r.tipologia && lista.length
    ? '<div class="cobros-aviso" style="margin-top:10px;"><i class="ti ti-alert-triangle"></i> Sin tipología el vehículo <strong>no puede salir</strong> ni se le puede cobrar, y una vez que salga ya no se le puede asignar.</div>'
    : "";
...
/*
    La línea que dice cuánto lleva en muelle y contra qué meta.

    Se muestra SIEMPRE que haya meta, no solo cuando se pasa: el
    supervisor necesita ver que un vehículo va en 40 de 105
    minutos para saber que va bien, no solo enterarse cuando ya
    es tarde.

    Cuando no hay meta lo dice en voz alta en vez de callar. Un
    muelle sin cifra se lee como "va bien", y lo que pasa es que
    ese vehículo no tiene tipología asignada — que es justo lo
    que alguien tiene que ir a corregir.
*/
...
function iniciarEdicion() {
  ...
  document.getElementById("btn-confirmar-editar").addEventListener("click", confirmarCorreccion);

  document.body.addEventListener("click", (e) => {
    const btn = e.target.closest("[data-editar]");
    if (btn) abrirModalEditar(btn.getAttribute("data-editar"));
  });
}
...
/* =========================================================
   ...
   Solo existe en J4: es la bodega cuyo supervisor responde por el
   registro completo, y las reglas de Firestore le abren la
   escritura únicamente a él (ver `bodegasConSupervisorEditor` en
   firestore.rules). En J3 y B9 este modal no está en el HTML.

   Lo que se corrige aquí es lo que se digitó mal en la entrada.
   El avance, las autorizaciones, los cobros y las cancelaciones
   siguen por su propio flujo: cada uno tiene validaciones que
   este formulario no hace.
   ========================================================= */

let editarVehiculoId = null;

/* Las horas se capturan en campos separados de hora y minuto, igual
   que en la portería: los <input type="time"> nativos muestran
   am/pm o 24h según el sistema operativo del dispositivo, y eso no
   se puede forzar desde la página. */
function partirHora(iso) {
  if (!iso) return { fecha: "", h: "", m: "" };
  const partes = String(iso).split("T");
  const hm = (partes[1] || "").slice(0, 5).split(":");
...
function armarFechaHora(fecha, horaId, minId) {
  const hh = dosDigitos(document.getElementById(horaId).value, 23);
  const mm = dosDigitos(document.getElementById(minId).value, 59);
  if (!fecha || hh === null || mm === null) return null;
...
function abrirModalEditar(id) {
  ...
  document.getElementById("editar-info").innerHTML =
    `<strong>${escapar(rec.placa)}</strong> — ${escapar(getDestino(rec))}`;

  // Los dos campos libres se rotulan como los llame esta bodega:
  // aquí se corrige el proveedor y el número de cita, no un
  // conductor con su cédula.
  document.getElementById("lbl-ed-conductor").textContent = etiquetaCampo(configBodega, "conductor");
  document.getElementById("lbl-ed-cedula").textContent = etiquetaCampo(configBodega, "cedula");
  ...
  pintarSelectTipologia(rec.tipologia);

  const entrada = partirHora(rec.horaEntrada);
  ...
  const cita = partirHora(rec.horaProgramacion);
  document.getElementById("ed-cita-h").value = cita.h;
  document.getElementById("ed-cita-m").value = cita.m;

  document.getElementById("editar-errores").style.display = "none";
  document.getElementById("modal-editar").classList.add("open");
}

/* La lista sale de la configuración de la bodega. Se agrega la
   tipología que ya trae el vehículo aunque el administrador la
   haya borrado después: si no, abrir el modal y guardar sin tocar
   nada le cambiaría la tipología al vehículo en silencio. */
function pintarSelectTipologia(seleccionada) {

  const sel = document.getElementById("ed-tipologia");
  const lista = (configBodega && configBodega.tipologias) || [];

  let html = '<option value="">Sin tipología</option>';
  ...
  const rec = registros.find((r) => r.id === editarVehiculoId);
  if (seleccionada && !encontrada) {
    html += `<option value="${escapar(seleccionada)}">${escapar((rec && rec.tipologiaNombre) || "Tipología retirada")}</option>`;
  }
...
async function confirmarCorreccion() {
  ...
  const conductor = document.getElementById("ed-conductor").value.trim();
  if (!conductor) errores.push(etiquetaCampo(configBodega, "conductor") + " no puede quedar vacío.");

  const fechaEntrada = document.getElementById("ed-fecha-entrada").value;
  const horaEntrada = armarFechaHora(fechaEntrada, "ed-hora-h", "ed-hora-m");
  ...
  if (horaEntrada && rec.horaSalida && new Date(rec.horaSalida) < new Date(horaEntrada)) {
    errores.push("La hora de entrada no puede ser posterior a la de salida (" + formatearFecha(rec.horaSalida) + ").");
  }

  const programado = document.getElementById("ed-programado").value === "Programado";
  const citaHora = dosDigitos(document.getElementById("ed-cita-h").value, 23);
  const citaMin = dosDigitos(document.getElementById("ed-cita-m").value, 59);
  const hayCita = citaHora !== null && citaMin !== null;

  if (programado && !hayCita) errores.push("Un vehículo programado necesita hora de cita.");

  if (errores.length) {
    const caja = document.getElementById("editar-errores");
    caja.innerHTML = errores.map((e) => "<div>• " + escapar(e) + "</div>").join("");
    caja.style.display = "";
    return;
  }
  ...
  const tipologiaId = document.getElementById("ed-tipologia").value;
  const tipologia = buscarTipologia(configBodega, tipologiaId);
  ...
  try {
    // La lista en vivo repinta sola con el cambio; este panel no
    // tiene toast, así que cerrar el modal ES la confirmación,
    // igual que en el cobro.
    await corregirRegistro(rec.id, rec, cambios, perfilActual.nombre);
    closeModal("modal-editar");
    editarVehiculoId = null;
  } catch (error) {
    console.error("Error al corregir el registro:", error);
    const caja = document.getElementById("editar-errores");
    caja.innerHTML = "<div>• No se pudo guardar la corrección. Revisa tu conexión e inténtalo de nuevo.</div>";
    caja.style.display = "";
  } finally {
    btn.disabled = false;
  }
...
function renderDashboard() {
  ...
  const enAlerta = enPatio.filter((r) => nivelPrioridad(minutosEsperando(r), umbralesDePatio()) === "alta");
...
function filaPatio(r) {
  ...
  const nivel = nivelPrioridad(min, umbralesDePatio());
  return `
    <tr>
      <td><strong>${escapar(r.placa)}</strong></td>
      <td>${escapar(r.conductor || "—")}</td>
...
function openModalNovedades(id) {
  ...
  document.getElementById("modal-novedades-body").innerHTML =
    fichaVehiculo(rec, {
      ...
      cobro: bodegaCobra() ? cobros[rec.id] || null : null,

      // Y si todavía no se ha cobrado, que lo diga. Antes no se
      // pasaba `mostrarPago` y un vehículo recién registrado no
      // mostraba NADA sobre el pago — ni el desglose, que no existe
      // aún, ni el aviso de que está pendiente. El bloque
...
function renderEstadisticas() {
  ...
  renderPanelEstadisticas({
    ...
    // La meta de patio contra la que se mide la pérdida de
    // operación sale de aquí, no de un número fijo en el módulo.
    config: configBodega,

    // El bloque de caja solo se pinta donde llegan los cobros, y
    // solo llegan aquí y al administrador: las reglas de Firestore
    // no le abren la colección al operario ni al cliente. Donde la
    // bodega no cobra no se manda nada y el bloque queda vacío.
    cobros: bodegaCobra() ? cobros : null
  });
}
...
function exportar() {
  ...
  // Los dos campos libres salen con el nombre que les da esta
  // bodega. Sin esto la hoja diría "Conductor" y "Cédula" en una
  // operación que anota proveedores y números de cita, y quien
  // reciba el archivo leería mal dos columnas enteras.
  const rotuloConductor = etiquetaCampo(configBodega, "conductor");
  const rotuloCedula = etiquetaCampo(configBodega, "cedula");

  const filas = filtrados.map((r) => ({
    Placa: r.placa,
    [rotuloConductor]: r.conductor || "",
    [rotuloCedula]: r.cedula || "",
